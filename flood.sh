#!/usr/bin/env bash
# Safe for curl|bash. Avoids CRLF surprises and dash-incompatible options.
set -eu

############################################
# Loranet/Bivocom Quick Setup (Ubuntu/Debian)
# - Node 18 via NVM (per target user)
# - Node-RED + palettes
# - Systemd service
# - Tailscale install
# - Serial access tweaks
############################################

# ---- User-adjustable defaults (can override via env) ----
: "${NODE_VERSION:=18}"
: "${NVM_VERSION:=v0.39.7}"
: "${NR_SERVICE:=node-red}"

# Target user: prefer SUDO_USER; fallback to current USER
...
say "Home: ${NR_HOME}"
say "Node via NVM: Node ${NODE_VERSION}, NVM ${NVM_VERSION}"
...
# ---- Groups for serial access ----
say "Ensuring ${NR_USER} is in 'dialout' group (serial permissions)..."
...
say "Node-RED started. UI: http://<this-host>:1880  |  Dashboard: /ui"
...
if [ -r /etc/os-release ]; then
  ...
  die "/etc/os-release not found; cannot determine codename for Tailscale repo."
fi
...
say "Tailscale daemon is running. Join your tailnet with:  sudo tailscale up --ssh"

# ---- Optional: udev hints for stable serial names ----
